...
* 记住账号切换：通过游戏登录页里的已记住账号列表进行 OCR 选择

为了降低侵入性，多账户功能不会读取、写入或恢复游戏注册表设置，也不会再保存启动器快照。
如果你想让不同账号使用不同分辨率或窗口模式，请在对应账号里填写启动参数，例如：

* `-screen-width 1920 -screen-height 1080 -screen-fullscreen 0`

启动页和多账户页面都会优先尝试自动预填常见游戏路径：

* 国服官服：`*\\miHoYo Launcher\\games\\Genshin Impact Game\\YuanShen.exe`
* 国际服：`*\\HoYoPlay\\games\\Genshin Impact game\\GenshinImpact.exe`

如果没有自动找到，请再手动点击“浏览”选择即可。
